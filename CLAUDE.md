...
## תכנון פלטפורמת SaaS עתידית (PassFlow)

יש דיון תכנוני (חלקו כבר יצא לפועל, ראו "הרצה מרחוק על Contabo" למטה) על
הפיכת הבוט הזה למוצר שנמכר לעסקי סוכנויות דרכונים אחרים - מודל עסקי,
ארכיטקטורת multi-tenant, שכבת אחסון/אבטחה, מאגר מכונות VPS/RDP להרצת
הדפדפן, טיפול בתקלות ושכבות self-service. **כל התכנון המלא נמצא ב-
`PLATFORM_PLAN.md`** באותה תיקייה - לקרוא שם לפני שממשיכים בנושא הזה.

## הרצה מרחוק על Contabo - proof of concept חי (2026-09-05 עד 2026-09-14)

**מטרה**: לבדוק אם אפשר להריץ את הבוט על מכונת Contabo (VPS עם RDP, כבר
קיים) **בלי** שמישהו יתחבר וילחץ שם משהו, ובלי לגעת ב-`run_autofill.py`/
`launcher.py` הקיימים. **אושש בפועל, חי, כמה פעמים.**

**מנגנון**: `worker_agent.py` (קובץ חדש) - סוכן שרץ על המכונה (Contabo,
בהמשך גם מכונות נוספות) ובודק כל 20 שניות שיטס-תור משותף ("AUTOFILL - Job
Queue", `11Aj96yzN8TZpDJuE...`, טאב `Queue`) - שורה עם `status=pending`
גורמת לו לקרוא ל-`launcher.run_with_sheet()` **הקיימת ולא-משתנה** (אותה
פונקציה שכפתור `passportbot://` כבר משתמש בה). מדווח גם "אני בחיים" לטאב
`Workers` (שם-מחשב/זמן-אחרון-נראה/עסוק-או-פנוי) - כך שאפשר לדעת אילו
מכונות קיימות בלי לגעת בהן בכלל. עבודות שנתקעות על "running" מעל 45 דקות
(מכונה שקרסה) מוחזרות לבד ל-`pending` כדי שמכונה אחרת תיקח אותן.

**למה polling מהמכונה ולא פקודה שנדחפת אליה**: אין חשיפת רשת נכנסת
(inbound) על המכונה, וקריטי יותר - זה שומר הכל בתוך session אינטראקטיבי
מחובר-בפועל (Startup folder, לא Windows Service/Task Scheduler
"run whether logged in or not") - שירות/Service רץ ב-Session 0 הנסתר,
ששם Chrome לא מקבל דסקטופ אמיתי - זה היה שובר בדיוק את ההתנהגות
headed-browser שגורמת לבדיקת ה-Cloudflare של האתר לעבור היום (ראו
[[feedback-anti-bot-pacing]]).

**הפעלה בפועל ללקוח**: כל שיטס AUTOFILL **חדש** (שנוצר דרך כפתור "לקוח
חדש" בזוהו) מקבל אוטומטית קישור בתא `Applicants!I1`: "▶ הרץ (Contabo -
גיבוי אם האוטופיל לא עובד)" - **בנוסף** לכפתור הקיים, לא במקומו (לפי בקשת
המשתמש - שיטסים קיימים לא מעודכנים רטרואקטיבית). לחיצה פותחת קישור https
רגיל ל-endpoint חדש (`/enqueue` ב-`autofill-intake-service`, סוד נפרד
וצר-היקף `autofill-enqueue-shared-secret` - לא אותו סוד כמו
`/scan_client_documents`, כדי שדליפה מתוך נוסחה גלויה בשיטס לא תיתן גישה
לשום דבר אחר) שרק כותב שורה לתור - שום Apps Script בכלל.

**⚠️ תגלית קריטית שגרמה לתיקון בפרויקט האחר**: הניסיון הראשון היה לשתול
את הקישור בתוך ה-Web App הישן של Apps Script בתבנית (`_handleNewClientRequest`
ב-`AppsScript_Code.gs`) - **התברר שזה קוד מת**. הזרימה האמיתית שזוהו קורא
לה היום היא `/new_autofill_client` ב-`trigger_service.py` **בפרויקט
document-analyzer** (`C:\Users\office_americandocs\document-analyzer`) -
מעתיקה את התבנית ישירות ב-Python דרך OAuth, בלי לגעת ב-Web App בכלל.
**התיקון האמיתי נכתב שם** - פונקציה נפרדת `_get_sheets_client()` +
כתיבת הנוסחה מיד אחרי `drive.files().copy()`, עטופה ב-try/except
שלעולם לא מפילה את יצירת הלקוח עצמה. נבדק חי על תיקיית `_TEST` - גם
יצירת הלקוח וגם הקישור עובדים, ה-IAM הציבורי של `trigger-service` (משותף
עם `/trigger` של ניתוח דוחות המס) נשאר בדיוק כמו שהיה. קוד ה-Apps Script
ב-`AppsScript_Code.gs` **נשאר בפועל** (לא נמחק) אבל הוא לא רץ.

**חוסן שנוסף ל-`build_driver()`/`_build_driver_with_retry()` (2026-09-14,
כולם ב-`run_autofill.py`) - שרשרת שלמה שנבנתה תוך כדי אבחון תקלה אמיתית
חוזרת ("chrome not reachable"/`SessionNotCreatedException`, stacktrace
אטום) על מחשב עובדת ספציפי:**
1. `_clear_stale_profile_locks` - מוחקת `SingletonLock`/`SingletonCookie`/
   `SingletonSocket` לפני כל הרצה (לא פתר את המקרה הזה בפועל, אבל תיקון
   נכון בפני עצמו).
2. `_build_driver_with_retry` - עד 3 ניסיונות עם 10 שניות המתנה, למקרה של
   מרוץ בין `driver.quit()` של מועמד קודם לפתיחת הדפדפן של הבא (לא זה מה
   שקרה בפועל הפעם - הכשל היה כבר על המועמד **הראשון** בהרצה).
3. `_kill_orphaned_chrome_processes` - מאתרת ומסיימת בכוח כל תהליך
   `chrome`/`chromedriver` שפקודת-ההפעלה שלו מכילה את נתיב הפרופיל
   הייעודי של הבוט (**לעולם לא** נוגעת בחלון Chrome רגיל של המשתמש) -
   גם זה לא פתר בפועל.
4. **מה שכן פתר בפועל**: מחיקת **כל** תיקיית `chrome_profile` (לא רק
   קבצי-נעילה) - אושר ידנית על המחשב הבעייתי. הפרופיל היה **פגום מבפנים**
   מקריסה ישנה, לא רק "נעול". `_build_driver_with_retry` עכשיו עושה את
   זה **אוטומטית** כמוצא אחרון, אחרי שכל השאר נכשל - אין יותר צורך
   בהתערבות ידנית ב-PowerShell כשזה קורה שוב.
   **אגב תיקון נפרד ומאושר באותו יום**: `undetected-chromedriver`/`selenium`
   לא היו הבעיה כאן (גרסת chromedriver שכבר הותקנה, 153.0.8010.36, כמעט
   זהה לגרסת Chrome המותקנת, 153.0.8010.37) - אבל `pip install --upgrade`
   שלהם נשאר צעד אבחון סביר לנסות ראשון בפעם הבאה שיש חשד לאי-התאמת-גרסה.

**⚠️ פער אמיתי שנמצא בנוסחת הזכאות הדינמית ב-`build_google_sheet_template.py`
(2026-09-10) - טרם תוקן בפועל**: `ds11_required` (הצביעה הדינמית שקובעת
מתי DS-11 מלא נדרש) **לא** כוללת את המקרה "הדרכון הקודם היה בעל תוקף
מוגבל (Limited Validity Under 2 Years)" - לפי הכללים האמיתיים, דרכון
קודם עם תוקף מוגבל פוסל מ-DS-82 בדיוק כמו Book Damaged/Lost/Stolen,
אבל השדה `limited_validity_cell` היום משמש רק לקבוע אילו שדות **אחרים**
חובה, לא נכנס בכלל לרשימת ה-`OR(...)` שקובעת את הדרישה ל-DS-11 עצמו.
צריך להוסיף `{limited_validity_cell}="Yes"` לרשימה ולהריץ מחדש
`build_google_sheet_template.py` כדי שהתבנית החיה תתעדכן (עותקים קיימים
לא יושפעו רטרואקטיבית, כרגיל).

## הערה על git

הריפו הזה אותחל תחת git ב-2026-08-11 (מקומי בלבד באותו שלב). **מ-2026-09
יש גם remote פרטי**: `https://github.com/office72/autofill-ds` - קומיטים
נדחפים (`git push`) אחרי אישור מפורש בכל פעם. `.gitignore` מוציא:
`service_account.json` (סוד),
